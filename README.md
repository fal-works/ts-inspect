...
An `Inspector<TResult>` has three main components:

1. **`name`**: Identifier for the inspector (e.g., "console-log-inspector")
2. **`nodeInspectorFactory`**: Given a `ts.SourceFile`, returns a `NodeInspector` function called for each AST node in depth-first order. Accumulates findings across nodes.
3. **`resultsBuilder`**: Processes all per-file raw results and returns structured diagnostic data as `InspectorResult`.
